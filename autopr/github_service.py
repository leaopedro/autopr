...
def start_work_on_issue(issue_number: int, repo_path: str = "."):
    ...
    try:
        # Fetch issue details using the more detailed function
        ...
        print(f"Successfully created branch '{branch_name}' and stored issue context.")
    except subprocess.CalledProcessError as e:
        ...
        print(f"An unexpected error occurred while starting work on issue: {e}")


def get_pr_changes(pr_number: int) -> str:
    """
    Fetches the changes (diff) for a given PR number.
    
    Args:
        pr_number: The number of the PR to fetch changes for.
        
    Returns:
        The diff string if successful, empty string if failed.
    """
    try:
        ...
        print(f"Error fetching PR changes:")
        print(f"Command '{' '.join(e.cmd)}' failed with exit code {e.returncode}")
        if e.stdout:
            print(f"Stdout:\n{e.stdout}")
        if e.stderr:
...
def post_pr_review_comment(pr_number: int, body: str, path: str, line: int) -> bool:
    """
    Posts a review comment on a specific line of a PR.
    ...
    try:
        ...
        print(f"Error posting review comment:")
        print(f"Command '{' '.join(e.cmd)}' failed with exit code {e.returncode}")
        if e.stdout:
            print(f"Stdout:\n{e.stdout}")
        if e.stderr:
