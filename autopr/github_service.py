...
def get_issue_details(issue_number: int) -> dict | None:
    ...
    try:
        gh_issue_cmd = [
            'gh', 'issue', 'view',
            str(issue_number),
            '--json', 'number,title,body,labels' # Added body and labels
        ]
        ...
        print(f"Error: Could not parse issue details for #{issue_number} from gh CLI output.")
        return None
    except Exception as e:
...
def get_commit_messages_for_branch(base_branch: str) -> list[str] | None:
    ...
    print(f"Fetching commit messages for current branch against base '{base_branch}'...")
    try:
        ...
        if result.stdout:
            ...
            return [] # No commits found, or an empty list if the command ran but no output
    except subprocess.CalledProcessError as e:
        ...
        if "unknown revision" in e.stderr.lower() or "not a valid object name" in e.stderr.lower():
            print(f"Hint: Ensure '{base_branch}' is a valid branch and an ancestor of the current branch.")
        return None
...
def start_work_on_issue(issue_number: int, repo_path: str = "."):
    ...
    try:
        ...
        if not os.path.isdir(git_dir_path):
            print(f"Error: .git directory not found at {git_dir_path}. Are you in a git repository?")
            return
        ...
        print(f"Error during 'workon' process (git checkout) for issue #{issue_number}:" )
        print(f"Command '{' '.join(e.cmd)}' failed with exit code {e.returncode}")
        if e.stdout: print(f"Stdout:\n{e.stdout}")
        if e.stderr: print(f"Stderr:\n{e.stderr}")
...
def create_pr_gh(title: str, body: str, base_branch: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            if process.stdout.strip(): # Sometimes gh sends info to stdout even on error
                error_message += f"\nstdout: {process.stdout.strip()}"
            return False, error_message
    except FileNotFoundError:
        return False, "Error: 'gh' command not found. Please ensure it is installed and in your PATH."
    except Exception as e:
        return False, f"An unexpected error occurred while trying to create PR with gh: {e}"
