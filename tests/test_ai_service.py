...
class TestGetPrDescriptionSuggestion(unittest.TestCase):
    ...
    def test_get_pr_description_suggestion_success(self, mock_openai_client):
        ...
        self.assertNotIn("Issue #", called_kwargs['messages'][1]['content']) # Ensure issue details are not in prompt
    ...
    @patch('autopr.ai_service.client')
    def test_get_pr_description_suggestion_cleans_title_and_body(self, mock_openai_client):
        mock_completion_choice = MagicMock()
        # Title with backticks and quotes, Body with triple backticks
        mock_completion_choice.message.content = '\"`Clean This Title`\"\n```markdown\nClean This Body\n```'
        mock_completion = MagicMock()
        mock_completion.choices = [mock_completion_choice]
        mock_openai_client.chat.completions.create.return_value = mock_completion
    ...
    @patch('autopr.ai_service.client')
    def test_get_pr_description_suggestion_body_only_single_backticks(self, mock_openai_client):
        mock_completion_choice = MagicMock()
